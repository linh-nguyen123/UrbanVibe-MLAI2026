...
from engine.dsp_filter import compute_db, apply_highpass_filter
...
class AudioAIEngine:
    ...
    def __init__(self):
        ...
        print(f"[UrbanVibe Engine] YAMNet khoi tao thanh cong voi {len(self.class_names)} nhan am thanh.")
    ...
    def infer(
        ...
        if apply_wind_filter and len(waveform_16k) > 128:
            try:
                proc_waveform = apply_highpass_filter(waveform_16k, sample_rate=16000, cutoff=300.0)
            except Exception:
                proc_waveform = waveform_16k

        # 3. Chạy suy luận qua mô hình YAMNet
        waveform_tensor = self.tf.convert_to_tensor(proc_waveform, dtype=self.tf.float32)
        ...
        max_scores = np.max(scores_np, axis=0) if len(scores_np.shape) > 1 else scores_np
